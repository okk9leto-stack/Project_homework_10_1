...
def test_log_negative_console(capsys: Any) -> None:
    """
    Тестирование ошибок для декоратора log
    Тест на провал: при некорректных аргументах декоратор ловит TypeError, ValueError и другие ошибки,
    выводит в консоль
    """
...
def test_log_file_negative() -> None:
    """
    Тестирование ошибок для декоратора log
    Тест на провал: файл задан, при некорректных аргументах декоратор ловит TypeError, ValueError
    и другие ошибки и выводит в файл
    """
    # 1. Получаем путь к директории, в которой находится текущий файл
    current_directory = os.path.dirname(__file__)
    # 2. Формируем путь к файлу test_mylog.txt
