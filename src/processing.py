...
def filter_by_state(dict_state: List[Dict], state: str = "EXECUTED") -> List[Dict]:
    """
    Функция принимает список словарей и значение для ключа (по умолчанию 'EXECUTED'),
    ...
    if dict_state == []:
        raise ValueError("Ошибка: отсутствие словарей")

    new_dict_state = []
    list_status = []

    for i, dict_i in enumerate(dict_state):
        list_status.append(dict_i["state"])

        if dict_i["state"] == state:
            new_dict_state.append(dict_i)

    if state not in list_status:
        raise ValueError("Ошибка: статус отсутствует или указан неверно")
...
def sort_by_date(dict_state: List[Dict], sort_date: bool = True) -> List[Dict]:
    ...
    for i, dict_i in enumerate(dict_state):
        if "date" not in dict_i:
            raise KeyError("Ошибка: дата")
...
#         {'id': 939719570, 'state': 'EXECUTED', 'date': '2019-09-12T02:08:58.425572'},
#         {'id': 939719575, 'state': 'EXECUTED', 'date': '31-10-2018T02:08:58.425572'},
# ]
